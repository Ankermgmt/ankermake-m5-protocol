...
def app_api_ankerctl_config_upload():
    ...
    if request.method != "POST":
        return web.util.flash_redirect_root()
    if "login_file" not in request.files:
        return web.util.flash_redirect_root("No file found", "danger")
    ...
    try:
        ...
        return web.util.flash_redirect_root(f"Error: {err}", "danger")
    except Exception as err:
        log.exception(f"Config import failed: {err}")
        return web.util.flash_redirect_root(f"Unexpected Error occurred: {err}", "danger")
...
def app_api_ankerctl_server_reload():
    ...
    with config.open() as cfg:
        ...
        if not cfg:
            return web.util.flash_redirect_root("No printers found in config", "warning")
        ...
        try:
            ...
            log.exception(err)
            return web.util.flash_redirect_root(f"Ankerctl could not be reloaded: {err}", "danger")

        return web.util.flash_redirect_root("Ankerctl reloaded successfully", "success")
...
def app_api_ankerctl_file_upload():
    ...
    try:
        ...
        return web.util.flash_redirect_root(f"File {file.filename} sent to printer!", "success")
    except ConnectionError as err:
        return web.util.flash_redirect_root(
            "Cannot connect to printer!\n"
            "Please verify that printer is online, and on the same network as ankerctl.\n"
            f"Exception information: {err}",
            "danger"
        )
    except Exception as err:
        return web.util.flash_redirect_root(f"Unknown error occurred: {err}", "danger")
